...
def main():
    ...
    query = 'SELECT name,lat,lon,elevation FROM measurementDescriptions WHERE type = "Wind"'
    cursor = cnx.cursor()
    ...
    for row in cursor.fetchall():
        if row[0] == 'Total Wind':
            continue
        loc_dict[row[0]] = {'lat':float(row[1]),'lon':float(row[2]),'elevation':float(row[3])}
    cursor.close()
            ...
            for orientation in ('v','h'):
                for i in range(0,wspd.shape[0]):
                    ...
                    plt.text(.65,-.15,'Initialized:'+init.strftime('%y/%m/%d %H:%M:%S')+
                             ' Valid:'+(init+datetime.timedelta(hours=i)).strftime('%y/%m/%d %H:%M:%S')
                             ,transform =ax.transAxes,backgroundcolor='w')
                    figname = '%s/%s/%s.png' % (figure_path,orientation,(init+datetime.timedelta(hours=i)).strftime('%y%m%d%H%M%S'))
                    plt.savefig(figname)
                    plt.close(fig)
        break
